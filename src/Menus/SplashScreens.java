...
/**
 * The SplashScreen class uses the entity modifiers and resolution-independent
		...
		public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
			if(pSceneTouchEvent.isActionDown()) {
				ResourceManager.getActivity().runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						final Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://twitter.com/ronbnaz"));
		...
		public boolean onAreaTouched(final TouchEvent pSceneTouchEvent, final float pTouchAreaLocalX, final float pTouchAreaLocalY) {
			if(pSceneTouchEvent.isActionDown()) {
				ResourceManager.getActivity().runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						final Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://andengine.org"));
						browserIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
						ResourceManager.getContext().startActivity(browserIntent);
					}
				});
	...
	public void onLoadScene() {
		...
		SplashScreens.mIFLLogo_SequenceEntityModifier.addModifierListener(new IModifierListener<IEntity>() {
			...
			public void onModifierFinished(final IModifier<IEntity> pModifier, final IEntity pItem) {
				...
				SplashScreens.mDisplayInfoText3.setText("Copyright 2014");
			}
