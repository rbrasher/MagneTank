...
/**
 * The MTrakk class creates and controls the vehicle that the game is
...
public class MTrakk implements IUpdateHandler {
	...
	private final BoundTouchInput mVehicleMovementBoundTouchInput = new BoundTouchInput() {
		...
		protected boolean onActionDown(final float pX, final float pY) {
			if(MathUtils.distance(pX, pY, MTrakk.this.mVehicleSprite.getX(), MTrakk.this.mVehicleSprite.getY()) > Math.max(MTrakk.mVEHICLE_MOVE_MAXIMUM_TOUCH_DISTANCE_FROM_VEHICLE, MTrakk.mVEHICLE_MOVE_MAXIMUM_TOUCH_DISTANCE_FROM_VEHICLE / ResourceManager.getCamera().getZoomFactor()))
				return false;
			return true;
		}
		...
		protected void onActionMove(final float pX, final float pY, final float pPreviousX, final float pPreviousY, final float pStartX, final float pStartY) {
			if(pX < pStartX)
				MTrakk.this.mWheelAngularVelocity = Math.min((pStartX - pX) / mVEHICLE_MOVE_INPUT_SENSITIVITY, mWHEEL_ANGULAR_VELOCITY_MAX);
			else
				MTrakk.this.mWheelAngularVelocity = Math.max((pStartX - pX) / mVEHICLE_MOVE_INPUT_SENSITIVITY, -mWHEEL_ANGULAR_VELOCITY_MAX);
		}
		...
		protected void onActionUp(final float pX, final float pY, final float pPreviousX, final float pPreviousY, final float pStartX, final float pStartY) {
			MTrakk.this.mWheelAngularVelocity = 0f;
			MTrakk.this.mLeftWheelBody.setAngularVelocity(0f);
			MTrakk.this.mRightWheelBody.setAngularVelocity(0f);
		}
	};
	...
	private final BoundTouchInput mTurretMovementBoundTouchInput = new BoundTouchInput() {
		...
		@Override
		protected boolean onActionDown(final float pX, final float pY) {
			this.PreviousTurretAngle = MTrakk.this.mTurretRevoluteJoint.getJointAngle();
			final float touchAngleFromTurret = ((float) Math.atan2((pY / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT) - MTrakk.this.mTurretRevoluteJoint.getAnchorA().y, (pX / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT) - MTrakk.this.mTurretRevoluteJoint.getAnchorA().x));
			MTrakk.this.setMangeTankTurretDesiredAngle(touchAngleFromTurret);
			return true;
		}
		...
		protected void onActionMove(final float pX, final float pY, final float pPreviousX, final float pPreviousY, final float pStartX, final float pStartY) {
			final float touchAngleFromTurret = ((float) Math.atan2((pY / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT) - MTrakk.this.mTurretRevoluteJoint.getAnchorA().y, (pX / PhysicsConstants.PIXEL_TO_METER_RATIO_DEFAULT) - MTrakk.this.mTurretRevoluteJoint.getAnchorA().x));
			if(touchAngleFromTurret > (mTURRET_JOINT_LOWER_ANGLE + MTrakk.this.mVehicleBody.getAngle()) && touchAngleFromTurret < (mTURRET_JOINT_UPPER_ANGLE + MTrakk.this.mVehicleBody.getAngle()))
				MTrakk.this.setMangeTankTurretDesiredAngle(touchAngleFromTurret);
		}
		
		@Override
		protected void onActionUp(final float pX, final float pY, final float pPreviousX, final float pPreviousY, final float pStartX, final float pStartY) {
			if(MTrakk.this.mTurretRevoluteJoint.getJointAngle() < (this.PreviousTurretAngle + this.MaxTapTurretAngleChange))
				if(MTrakk.this.mTurretRevoluteJoint.getJointAngle() > (this.PreviousTurretAngle - this.MaxTapTurretAngleChange)) {
					...
					if(pX > (pStartX - (MaxTapDistanceScaled)))
						if(pX < (pStartX + (MaxTapDistanceScaled)))
							if(pY > (pStartY - (MaxTapDistanceScaled)))
								if(pY < (pStartY + (MaxTapDistanceScaled)))
									if(MTrakk.this.mTurretMagnetOn)
										MTrakk.this.requestShoot();
				}
		}
	};
	...
	// CONSTRUCTOR
	// ====================================================
	public MTrakk(final float pX, final float pY, final GameLevel pGameLevel) {
		...
		this.mShadowSprite = new Sprite(this.mSHADOW_START_X, this.mSHADOW_START_Y, ResourceManager.gameMTrakkShadowTR, ResourceManager.getActivity().getVertexBufferObjectManager());
		this.mGameLevel.attachChild(this.mShadowSprite);
		
		this.mVehicleSprite = new Sprite(this.mCHARACTER_START_X, this.mCHARACTER_START_Y, ResourceManager.gameMTrakkBodyTR, ResourceManager.getActivity().getVertexBufferObjectManager()) {
			@Override
			public void setPosition(final float pX, final float pY) {
				super.setPosition(pX, pY);
				MTrakk.this.mShadowSprite.setPosition(pX + mSHADOW_OFFSET_X, MTrakk.this.mShadowSprite.getY());
				MTrakk.this.updateShadowAlpha();
			}
		};
		this.mTurretSprite = new Sprite(this.mVehicleSprite.getX() + mTURRET_SPRITE_OFFSET_FROM_VEHICLE_CENTER_X, this.mVehicleSprite.getY() + mTURRET_SPRITE_OFFSET_FROM_VEHICLE_CENTER_Y, ResourceManager.gameMTrakkTurretTR, ResourceManager.getActivity().getVertexBufferObjectManager());
		...
		this.mLeftWheelSprite = new Sprite(this.mVehicleSprite.getX() - mWHEEL_HORIZONTAL_DISTANCE_FROM_VEHICLE_CENTER, this.mVehicleSprite.getY() - mWHEEL_VERTICAL_DISTANCE_FROM_VEHICLE_CENTER, ResourceManager.gameMTrakkWheelTR, ResourceManager.getActivity().getVertexBufferObjectManager());
		this.mRightWheelSprite = new Sprite(this.mVehicleSprite.getX() + mWHEEL_HORIZONTAL_DISTANCE_FROM_VEHICLE_CENTER, this.mVehicleSprite.getY() - mWHEEL_VERTICAL_DISTANCE_FROM_VEHICLE_CENTER, ResourceManager.gameMTrakkWheelTR, ResourceManager.getActivity().getVertexBufferObjectManager());
		this.mLeftWheelShadowSprite = new Sprite(this.mLeftWheelSprite.getWidth() / 2f, this.mLeftWheelSprite.getHeight() / 2f, ResourceManager.gameMTrakkWheelShadowTR, ResourceManager.getActivity().getVertexBufferObjectManager()) {
			@Override
			protected void onManagedUpdate(final float pSecondsElapsed) {
				super.onManagedUpdate(pSecondsElapsed);
				if(this.getRotation() != -MTrakk.this.mLeftWheelSprite.getRotation()) {
					this.setRotation(-MTrakk.this.mLeftWheelSprite.getRotation());
				}
			}
		};
		this.mRightWheelShadowSprite = new Sprite(this.mRightWheelSprite.getWidth() / 2f, this.mRightWheelSprite.getHeight() / 2f, ResourceManager.gameMTrakkWheelShadowTR, ResourceManager.getActivity().getVertexBufferObjectManager()) {
			@Override
			protected void onManagedUpdate(final float pSecondsElapsed) {
				super.onManagedUpdate(pSecondsElapsed);
				if(this.getRotation() != -MTrakk.this.mRightWheelSprite.getRotation()) {
					this.setRotation(-MTrakk.this.mRightWheelSprite.getRotation());
				}
			}
		};
		...
		mTurretConnectionSprite = new Sprite(0f,0f, ResourceManager.gameMTrakkTurretConnectionTR, ResourceManager.getActivity().getVertexBufferObjectManager()) {
			@Override
			protected void onManagedUpdate(final float pSecondsElapsed) {
				super.onManagedUpdate(pSecondsElapsed);
				if(this.getRotation() != MTrakk.this.mVehicleSprite.getRotation()) {
					this.setRotation(MTrakk.this.mVehicleSprite.getRotation());
				}
				if((this.getX() != MTrakk.this.mVehicleSprite.getX()) || (this.getY() != MTrakk.this.mVehicleSprite.getY())) {
					this.setPosition(MTrakk.this.mVehicleSprite);
				}
			}
		};
		...
		final MassData MTrakkTurretMassData = this.mTurretBody.getMassData();
		MTrakkTurretMassData.center.set(mTURRET_BODY_CENTER_OF_GRAVITY_X, mTURRET_BODY_CENTER_OF_GRAVITY_Y);
		this.mTurretBody.setMassData(MTrakkTurretMassData);
		
		final RevoluteJointDef MTrakkTurretRevoluteJointDef = new RevoluteJointDef();
		MTrakkTurretRevoluteJointDef.initialize(this.mVehicleBody, this.mTurretBody, this.mTurretBody.getWorldCenter());
		MTrakkTurretRevoluteJointDef.enableMotor = true;
		MTrakkTurretRevoluteJointDef.maxMotorTorque = mTURRET_JOINT_MAX_MOTOR_TORQUE;
		MTrakkTurretRevoluteJointDef.enableLimit = true;
		MTrakkTurretRevoluteJointDef.lowerAngle = mTURRET_JOINT_LOWER_ANGLE;
		MTrakkTurretRevoluteJointDef.upperAngle = mTURRET_JOINT_UPPER_ANGLE;
		this.mTurretRevoluteJoint = (RevoluteJoint) this.mGameLevel.mPhysicsWorld.createJoint(MTrakkTurretRevoluteJointDef);
	...
	public boolean onMTrakkTouchEvent(final Scene pScene, final TouchEvent pSceneTouchEvent) {
	...
	public void requestShoot() {
		if(this.mTurretMagnetOn) {
			...
			if(this.mGrabbedMagneticObject != null) {
				...
				final float shootingVolume = 0.75f * ((this.mShootingPower - MTrakk.mSHOOTING_POWER_MINIMUM) / (MTrakk.mSHOOTING_POWER_MAXIMUM - MTrakk.mSHOOTING_POWER_MINIMUM));
